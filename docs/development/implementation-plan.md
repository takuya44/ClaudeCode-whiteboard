...
#### 認証関連API
1. **プロフィール更新API** (`PUT /api/v1/auth/profile`)
   - 担当: バックエンド担当者A
   - 実装内容: ユーザー名、メールアドレスの更新
   - フロントエンド側: `auth.ts` の `updateProfile` メソッドで既に準備済み
   - 完了条件: ユーザー情報が更新され、更新後のユーザーデータがJSONで返される

2. **パスワード変更API** (`POST /api/v1/auth/change-password`)
   - 担当: バックエンド担当者A
   - 実装内容: 現在のパスワード確認と新パスワードへの変更
   - フロントエンド側: `ProfileView.vue` で UI は実装済み、API 呼び出し部分が TODO
...
#### Week 8 完了基準 🚧 **進行中 (2025-07-03)**
- [x] 基本機能統合完了
- [ ] API 実装完全統合（プロフィール更新、パスワード変更、共有機能）
...
**次のステップ**: 
1. 未実装 API の完成（プロフィール更新、パスワード変更、共有機能）
2. フロントエンドストア層の API 統合完了
3. WebSocket によるリアルタイム同期の完全実装
4. Phase 4 後半の統合テスト・品質向上作業

### 📝 TODO サマリー (2025-07-03)
- **バックエンド**: 5つの API エンドポイント実装（タスク1-5）
- **フロントエンド**: 4つのストアメソッド統合 + 3つの WebSocket 送信実装（タスク6-12）
- **バックエンド WebSocket**: 3つのリアルタイム機能実装（タスク13-15）

### 📈 実装優先順位
1. **バックエンド API** （タスク1-5）→ フロントエンド統合の前提条件
2. **フロントエンドストア統合** （タスク6-9）→ API 完成後即実装

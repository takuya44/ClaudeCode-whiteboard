...
CREATE INDEX idx_drawing_elements_whiteboard_id ON drawing_elements(whiteboard_id);
...
-- whiteboard_collaborators テーブル
CREATE INDEX idx_whiteboard_collaborators_user_id ON whiteboard_collaborators(user_id);
CREATE INDEX idx_whiteboard_collaborators_permission ON whiteboard_collaborators(permission);
```
